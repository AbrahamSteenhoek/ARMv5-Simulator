 ...
 */
bool RAM::TestFlag(uint addr, uint32_t bit)
{   if(addr + 4 > memsize) {
 ...
 */
void RAM::SetFlag(uint addr, uint32_t bit, bool flag)
{
    if(addr + 4 > memsize) {
 ...
 */
uint32_t RAM::ExtractBits(uint32_t word, uint32_t startBit, uint32_t endBit)
{
    uint extract = 0;
 ...
 */
bool RAM::WriteByte(uint addr, unsigned char byte)
{
    if (addr >= addr_space()) {
        //cerr << "Attempting to write outside of RAM space." << endl;
        return false;
    }
    memory[addr] = byte;
    return true;
 ...
 */
bool RAM::WriteHalfWord(uint addr, ushort halfWord)
{
    if(addr + 2 > memsize) {
 ...
 */
bool RAM::WriteWord(uint addr, uint32_t word)
{
    if(addr + 4 > memsize) {
 ...
 */
unsigned char RAM::ReadByte(uint addr)
{
    if (addr >= addr_space()) {
 ...
 *  - Quits the program if user attempts to read outside of available memory space
 */
ushort RAM::ReadHalfWord(uint addr)
{
    if(addr + 2 > memsize) {
 ...
 *  - Quits the program if user attempts to read outside of available memory space
 */
uint RAM::ReadWord(uint addr)
{
    if(addr + 4 > memsize) {
            cerr << "Attempting to read outside of available memory space with ReadWord method." << endl;
            exit(1);
    }
    if (addr % 4 == 0) {
    return memory[addr] | (memory[addr + 1] << 8) | (memory[addr + 2] << 16) | (memory[addr + 3] << 24);
        ...
        cerr << "ReadWord method required an address that is divisible by 4" << endl;
        exit(1);
    }
}
 ...
 */
int RAM::calcChecksum(int arr_len, unsigned char *array)
{
    int cksum = 0;
 ...
 */
RAM::RAM(QString memsize_QStr)
{
    if (memsize_QStr.isNull() || memsize_QStr == "") {
        memsize = DEF_MEM_SIZE;
    }
    else {
        if (!RAM::isPosNum(memsize_QStr)) {
