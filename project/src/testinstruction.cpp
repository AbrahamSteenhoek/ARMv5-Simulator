...
void testInstruction::runTests()
{
    ...
    // cmp r0, #3
    regs->set(0, 5);
    Instruction *cmp = new Instruction_DataProcessing(0xE3500003, testCPU->Regs(), testCPU->memory());
    cmp->exec();
    CPSR_temp = regs->getCPSR();
    assert((CPSR_temp & (1 << 29)) != 0);
    assert((CPSR_temp & (1 << 28)) == 0);

    // cmp r0, 0xff000000
    cmp = new Instruction_DataProcessing(0xE35004FF, regs, RAM);
    cmp->exec();
    assert((regs->getCPSR() & (1 << 29)) == 0);
    ...
    // mov r0, #5
    regs->set(0, 0);
    mov = new Instruction_DataProcessing(0x23A00005, regs, RAM);
    mov->exec();
    assert(regs->get(0) == 0);
    mov = new Instruction_DataProcessing(0x33A00005, regs, RAM);
    mov->exec();
    assert(regs->get(0) == 5);

    regs->set(0, 5);
    cmp = new Instruction_DataProcessing(0xE3500003, testCPU->Regs(), testCPU->memory());
    cmp->exec();
    mov = new Instruction_DataProcessing(0x23A00008, regs, RAM);
    mov->exec();
    assert(regs->get(0) == 8);

    cout << "mode: " << regs->curMode() << endl;
    // testing B, BL, BX
    //B
    regs->set(15, 0);
    Instruction *branch = new Instruction_Branch(0xEA800000, regs, RAM, 0);
    branch->exec();
    cout << "PC: " << regs->get(PC) << endl;
    //assert(regs->get(15) == -8388608);

    //BL
    regs->set(15, 0xFF);
    branch = new Instruction_Branch(0xEB00000C, regs, RAM, 0xFF);
    branch->exec();
    //assert(regs->get(15) == 0xFF + 0xC);
    //assert(regs->get(14) == 0xFF);

    //BX
    regs->set(15, 0x10);
    regs->set(3, 0x40);
    branch = new Instruction_Branch(0xE1200013, regs, RAM, 0x10);
    branch->exec();
    assert(regs->get(15) == (regs->get(3) & 0xFFFFFFFE));
    regs->set(3, -0x10);
    branch = new Instruction_Branch(0xE1200013, regs, RAM, regs->get(15));
    branch->exec();
    assert(regs->get(15) == (regs->get(3) & 0xFFFFFFFE));
    ...
    regs->set(4, 0x55);
    regs->setCPSR(0x6969);
    Instruction *mrs = new Instruction_DataProcessing(0xE1004000, regs, RAM);
    mrs->exec();
    assert(regs->get(4) == 0x6969);
    cmp = new Instruction_DataProcessing(0xE3500003, testCPU->Regs(), testCPU->memory());
    cmp->exec();
    assert(regs->getCPSR() != 0x6969);
    mrs->exec();
    assert(regs->get(4) == regs->getCPSR());
